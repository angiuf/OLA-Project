...
class GreedyAlgorithm:
    def __init__(self, prices, return_reward, n_products, n_arms):
        self.prices = prices
        self.return_reward = return_reward
        assert (n_products == len(prices))
        self.n_products = n_products
        self.n_arms = n_arms

    def optimization_algorithm(self):
        price_arm = np.zeros(self.n_products).astype('int')
